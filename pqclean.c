...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
    ...
    hash_to_point_ct(&sc, r.hm, 9, tmp.b);
     ...
     */
    for (;;) {
        sign_dyn(r.sig, &sc, f, g, F, G, r.hm, 9, tmp.b);
        v = comp_encode(sigbuf, *sigbuflen, r.sig, 9);
        if (v != 0) {
            inner_shake256_ctx_release(&sc);
            *sigbuflen = v;
            return 0;
...
do_verify(
    ...
    if (comp_decode(sig, 9, sigbuf, sigbuflen) != sigbuflen) {
        return -1;
    }
    ...
    inner_shake256_inject(&sc, m, mlen);
    inner_shake256_flip(&sc);
    hash_to_point_ct(&sc, hm, 9, tmp.b);
    inner_shake256_ctx_release(&sc);
